...
**เวอร์ชัน 1.0.0** · [CHANGELOG](CHANGELOG.md)
...
## 🚀 เริ่มต้น — โคลนโปรเจกต์ (ทำก่อนทุกอย่าง)
```bash
git clone https://github.com/Boom-Vitt/anglerfish-fable5
cd anglerfish-fable5
```
...
### ติดตั้ง
> 💡 แนะนำให้สร้าง **virtual environment** ก่อน (จำเป็นบน macOS/Homebrew ที่เป็น PEP 668 externally-managed มิฉะนั้น `pip install` จะ error)
```bash
python3 -m venv .venv
...
### รันบอต
```bash
python3 autotrade.py            # โหมดจำลอง + testnet (ปลอดภัย เริ่มที่นี่)
```
...
| `OPENROUTER_MODEL` | `anthropic/claude-fable-5` | โมเดล LLM (ใส่ slug อะไรก็ได้ของ OpenRouter) |
...
- 🧱 `index.html` เป็นไฟล์ที่ **build แล้ว** (self-contained) — ซอร์สโมดูลแบบแยกไฟล์อยู่ใน `build/` ซึ่ง**ไม่รวม**ในรีโปนี้ (ใช้แดชบอร์ดได้เลยโดยไม่ต้อง build)
- 🔑 เส้นทาง **เทรดจริง + OpenRouter ต้องใช้คีย์ของคุณเอง** — โค้ดครบและจัดการ error อย่างปลอดภัย (โหมดจำลองยืนยันแล้วว่าไม่ยิงออเดอร์จริง) แต่เส้นทาง LLM/เทรดจริงยังไม่ได้ทดสอบกับ API จริงในรุ่นนี้
